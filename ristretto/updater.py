...
from post import NotAPostException, PostIsDraftException, Post
import tools
...
class Updater(object):
    ...
    def __init__(self, settings, *args, **kwargs):
        ...
        # Init Jinja2
        self.j2 = Environment(loader=FileSystemLoader(self.s.TEMPLATES_DIR, 
            encoding=self.s.INPUT_ENCODING))


    def update(self, force_publish=False):

        self.publish_drafts_previews()
        ...
        # Check posts that need publishing or republishing
    ...
    def publish_drafts_previews(self):

        self.process_drafts()
        self.process_publish_now(publish_future=False)
    ...
    def is_in_future(self, post):
        """
        Compares post date with today's date (down to hours, minutes and seconds,
        ...
        Returns:
            True if post's date is in the future.
        """
    
        # compares without milliseconds
        today = datetime.datetime.today().replace(microsecond=0)
        post_date = post.date.replace(microsecond=0)
        
        return (post_date - today > datetime.timedelta(0))
    ...
    def get_all_posts_paths(self, posts_dir):
        """
        Returns a list of all posts paths.
        """
        result = []
    ...
    def remove_post_review(self, post):

        fname = os.path.join(self.s.DRAFTS_PREVIEW_DIR, 
                "%s%s" % (os.path.splitext(
                    os.path.split(post.fname)[1][0]), self.s.HTML_EXT)
            )
        os.remove(fname)
    ...
    def migrate(self):
        ...
        posts_fpaths = self.get_all_posts_paths(self.s.POSTS_DIR)
        pages_fpaths = self.get_all_posts_paths(self.s.PAGES_DIR)
        
        
        for f in posts_fpaths+pages_fpaths:
        
            print("Reading post... {0}".format(f))
            po = Post(fname=f, settings=self.s)
            pd = Post2(settings=self.s)
            ...
            pd.status = po.status
            pd.layout = po.type
            pd.title_link = po.title_link
            pd.tags = po.tags
            pd.categories = po.categories
            pd.content = po.content
            pd.permalink = po.permalink
            
            print("Converting post... file: {0}".format(f))
            pd.normalize()
...
class FullRSSItem(PyRSS2Gen.RSSItem):
    ...
    def publish_extensions(self, handler):
        handler._out.write('<%s><![CDATA[%s]]></%s>' % (
            "description", self.do_not_autooutput_description.encode('utf-8'), "description"
        ))
        if self.content:
            handler._out.write('<%s><![CDATA[%s]]></%s>' % (
                "content:encoded", self.content.encode('utf-8'), "content:encoded"
            ))
# http://stackoverflow.com/questions/5371704/python-generated-rss-outputting-raw-html/7912205#7912205
